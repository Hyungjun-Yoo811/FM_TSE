...
def main() -> None:
    ...
    model = maybe_wrap_model(STFTMaskTSE(**checkpoint["config"]["model"]), device, gpu_ids)
    ...
    with torch.no_grad():
        estimate_spec = model(codec.encode(mixture), codec.encode(enrollment))
        estimate = codec.decode(estimate_spec, length=target.shape[-1])
    ...
    save_waveform_panel(
        ...
        title=f"STFT Inference ({args.split}:{args.index})",
    )
